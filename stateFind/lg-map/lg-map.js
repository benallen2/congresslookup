      ...
      for (var state in paths) {
        ...
        hitArea.mouseup(function(e) {
          ...
          if (paths[id].enable) {
            ...
            if (config.useText) {
              $.ajax({//fetch the data on reps from sunlight based on clicked state
                url: "https://congress.api.sunlightfoundation.com/legislators?per_page=all&state="+ paths[id].abbreviation + "&apikey=dc06e77466b24aad948f211c8cdc4a29"
                })
                .done( (data) => {
                  textArea.html(paths[id].text);
                  textArea.append("<h3>Senators</h3>");
                  for (var j = 0; j < data.results.length; j++){
                    if (data.results[j].chamber == "senate" && data.results[j].party == "D"){
                      textArea.append("<p class='dParty rep' data-toggle='modal' data-target='#myModal' id=" + data.results[j].bioguide_id + ">" + data.results[j].first_name + " " + data.results[j].last_name + "</p>")
                    }
                    else if(data.results[j].chamber == "senate" && data.results[j].party == "R"){
                      textArea.append("<p class='rParty rep' data-toggle='modal' data-target='#myModal' id=" + data.results[j].bioguide_id + ">" + data.results[j].first_name + " " + data.results[j].last_name + "</p>")
                    }
                    else if(data.results[j].chamber == "senate" && data.results[j].party == "I"){
                      textArea.append("<p class='iParty rep' data-toggle='modal' data-target='#myModal' id=" + data.results[j].bioguide_id + ">" + data.results[j].first_name + " " + data.results[j].last_name + "</p>")
                    }
                  }
                  textArea.append("<h3>Representatives</h3>");
                  var reps = [];
                  for (var k = 0; k < data.results.length; k++){
                    if (data.results[k].chamber == "house"){
                      reps[k] = (data.results[k]);
                    }
                  }
                  reps.sort(function(a, b){
                    return a.district - b.district;
                  });
                  for (var l = 0; l < reps.length; l++){
                    if(reps[l].party == "D"){
                      textArea.append("<p class='dParty rep' data-toggle='modal' data-target='#myModal' id=" + reps[l].bioguide_id + ">District " + (l+1) + ": "+ reps[l].first_name + " " + reps[l].last_name + "</p>");
                    }
                    else if(reps[l].party == "R"){
                      textArea.append("<p class='rParty rep' data-toggle='modal' data-target='#myModal' id=" + reps[l].bioguide_id + ">District " + (l+1) + ": " + reps[l].first_name + " " + reps[l].last_name + "</p>");
                    }
                    else if(reps[l].party == "I"){
                      textArea.append("<p class='iParty rep' data-toggle='modal' data-target='#myModal' id=" + reps[l].bioguide_id + ">District " + (l+1) + ": " + reps[l].first_name + " " + reps[l].last_name + "</p>");
                    }
                    $(".rep").click(function(){// Setup for the modal here
